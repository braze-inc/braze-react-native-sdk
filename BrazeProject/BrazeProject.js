...
class BrazeProject extends Component {
  constructor(props) {
    super(props);
    this.state = {
      userIdText: '',
    ...
    this._requestNewsFeedRefresh = this._requestNewsFeedRefresh.bind(this);
    ...
    return (
      <ScrollView
        ...
        <TouchableHighlight
          ...
          <Text style={{ color: "red", fontWeight: "bold" }}>Flush Data ⚡️</Text>
        </TouchableHighlight>
        ...
        <Space />
        <TouchableHighlight
          onPress={this._requestNewsFeedRefresh}>
        ...
        <TouchableHighlight
          onPress={this._logNewsFeedInteractions}>
  ...
  _requestNewsFeedRefresh(event) {
    Braze.requestFeedRefresh();
    this._showToast('News Feed refreshed');
  }

  _logNewsFeedInteractions(event) {
    Braze.getNewsFeedCards().then(function (result) {
      if (result === undefined || result.length == 0) {
        console.log('No cached News Feed cards Found.');
      } else {
        console.log(result.length + ' cached News Feed card Foundssss.');
        for (const card of result) {
          var cardId = card.id;
          console.log('Logging click + impression for News Feed card: ' + JSON.stringify(result));
